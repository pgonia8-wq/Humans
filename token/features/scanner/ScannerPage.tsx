...
import { useTokenScanner, type TokenAnalysis, type RiskLevel, type TokenHeat } from "@/hooks/useTokenScanner";
...
function TokenAnalysisCard({ analysis, onViewToken }: { analysis: TokenAnalysis; onViewToken: (id: string) => void }) {
...
export default function ScannerPage() {
  const { analyses, marketSummary, scanning, lastScan, scanCount, rescan } = useTokenScanner();
      ...
      <div className="sticky top-0 z-20 bg-background/80 backdrop-blur-xl border-b border-border/30 px-4 pt-4 pb-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                {scanning ? "Escaneando..." : lastScan ? `Scan #${scanCount} · ${new Date(lastScan).toLocaleTimeString()}` : "Iniciando..."}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex gap-1">
          {(["overview", "rankings", "alerts"] as ViewMode[]).map(mode => (
            ...
            >
              {mode === "overview" ? "Resumen" : mode === "rankings" ? "Rankings" : `Alertas${alerts.length > 0 ? ` (${alerts.length})` : ""}`}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-3">
        {viewMode === "overview" && (
          <>
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-foreground">Top Tokens</p>
            </div>
        ...
        {scanning && analyses.length === 0 && (
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-sm text-muted-foreground">Analizando tokens...</p>
            <p className="text-[10px] text-muted-foreground/60">Detectando ballenas, patrones y riesgos</p>
          </div>
        )}
      </div>
    </div>
