...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: updatedLink, error: linkUpdateErr } = await supabase
      .from("airdrop_links")
      .update({
    ...
    if (linkUpdateErr || !updatedLink) {
      await supabase.from("airdrop_claims")
        .delete()
        .eq("airdrop_link_id", link.id)
        .eq("user_id", userId);
      console.warn("[AIRDROP] OCC conflict — concurrent claim detected, rolled back:", linkUpdateErr?.message);
