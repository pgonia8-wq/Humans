...
import { useLanguage } from "../../LanguageContext"; // <- importamos el contexto
...
const Inbox: React.FC<InboxProps> = ({ currentUserId, onClose }) => {
  const { t } = useLanguage(); // <- usamos t() para traducciones
  ...
  const loadConversations = async () => {
    ...
    const otherIds = convs.map(c => c.user1_id === currentUserId ? c.user2_id : c.user1_id);
  ...
  const openChat = (userId: string) => setChatUserId(userId);
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-full bg-gray-600 overflow-hidden flex items-center justify-center text-white">
          {p?.avatar_url ? <img src={p.avatar_url} className="w-full h-full object-cover" /> : p?.username?.[0]}
        </div>
        <span className="text-white text-sm">{p?.username || id.slice(0,6)}</span>
      </div>
    );
  };
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden">
      ...
      <div className="flex justify-between mb-3">
        <h2 className="text-white font-bold text-lg">{t("mensajes")}</h2>
        <button onClick={onClose} className="text-gray-400">{t("cerrar")}</button>
      </div>

      <input
        value={searchQuery}
        onChange={(e) => handleSearch(e.target.value)}
        placeholder={t("buscar_seguidores")}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          <p className="text-gray-400 text-center">{t("cargando")}</p>
        ) : (
          conversations.map(c => {
            ...
            return (
              ...
              >
                ...
                {unread > 0 && <span className="bg-red-600 text-xs px-2 rounded-full">{unread}</span>}
              </div>
            );
          })
        )}
